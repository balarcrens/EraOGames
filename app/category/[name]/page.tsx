...
export default function CategoryPage({ params }: { params: { name: string } }) {
  ...
  return (
    <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10 text-slate-800 dark:text-gray-100 font-sans">
      {/* Breadcrumbs */}
      <div className="flex items-center gap-2 text-xs md:text-sm text-slate-500 dark:text-slate-400 mb-6 font-medium">
      ...
      <div className="animate-fade-in-up animate-delay-100 flex flex-wrap gap-2.5 mb-8">
        <Link
          href="/category/all"
      ...
      {catGames.length > 0 ? (
        ...
        <div className="text-center py-20 bg-white dark:bg-[#121824] border border-slate-200 dark:border-slate-800 rounded-2xl">
          ...
          >
            <GamepadIcon className="w-8 h-8 text-slate-400 dark:text-slate-500" />
          </div>
          <h2 className="text-lg font-display font-bold text-slate-800 dark:text-white mb-1">No games found</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Check back soon for new games in this category.</p>
        </div>
      )}
    </div>
  );
